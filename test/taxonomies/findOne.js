...
describe('GET /taxonomies', function () {
    ...
    before(function (done) {
        pot.drop('taxonomies', function (err) {
            if (err) {
                return done(err);
            }
    ...
    var validatetaxonomies = function (taxonomies) {
        taxonomies.forEach(function (page) {
    ...
    it('invalid id', function (done) {
        request({
            uri: pot.resolve('apis', '/v/taxonomies/undefined'),
            method: 'GET',
            auth: {
    ...
    it('owner can access', function (done) {
        request({
            ...
            request({
                uri: pot.resolve('apis', '/v/taxonomies/' + b[0].id),
                method: 'GET',
                auth: {
                    bearer: client.users[0].token
                },
    ...
    it('can be accessed by anyone when public', function (done) {
        request({
            ...
            var page = b[0];
            request({
                uri: pot.resolve('apis', '/v/taxonomies/' + page.id),
                method: 'GET',
                auth: {
                    bearer: client.users[1].token
                },
                ...
                request({
                    ...
                    should.exist(b.message);
                    b.code.should.equal(errors.notFound().data.code);
                    pot.publish('taxonomies', page.id, client.users[0].token, client.admin.token, function (err) {
                        if (err) {
                            return done(err);
                        }
                        request({
